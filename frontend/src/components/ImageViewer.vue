...
const { currentImageDetail, currentFileDetails, currentFileDetail } = storeToRefs(currentImageStore)
...
watch(
  ...
  () => {
    if (currentFileDetails?.value?.length) {
      currentFileIndex.value = 0
      currentFileDetail.value = currentFileDetails.value[0] || null
    } else {
